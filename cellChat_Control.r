...
lv_ctl <- subset(lv,cells = which(lv$Condition=='Control'))

dim(lv_ctl)

#normalize data
lv_ctl<-NormalizeData(lv_ctl)
#extract normalized count matrix from seu object
data.input<-GetAssayData(lv_ctl,assay = "RNA",slot="data")
groups<-lv_ctl$CellType_correct_edit1
meta<-data.frame(group=groups,row.names = names(groups))

rm(lv)
gc()

cellchat <- createCellChat(object = data.input, meta = lv_ctl@meta.data, group.by = "CellType_correct_edit1")
levels(cellchat@idents)
groupSize <- as.numeric(table(cellchat@idents)) 
rm(lv_ctl,data.input)
gc()
...
#options(rePercentageFeatureSet.width = 8, repr.plot.height = 6, repr.plot.res = 200,ggrepel.max.overlaps = Inf)
...
cellchat = readRDS('output//LV_control.rds')
...
saveRDS(cellchat,'output/LV_control.rds')
...
saveRDS(cellchat, file = "output/LV_control.rds")
